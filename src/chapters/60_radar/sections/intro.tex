...
\Cref{sec:radar.problem} introduces the reader to the problem of model quality assessment in \gls{csfl} and the necessary background.
\Cref{sec:radar.related} reviews the related work, before we dive in \thecontrib's architecture in \Cref{sec:radar.archi}.
\Cref{sec:radar.methodo,sec:radar.results} present the experimental setup and results, and we discuss our findings in \Cref{sec:radar.discussion}.
